...
mv $HOME/DARK/files/chat.sh /data/data/com.termux/files/usr/bin/chat
...
echo
    ...
    sed "s/DARK-D/$name/g" "$INPUT_FILE" > "$HOME/.zshrc"
    sed "s/DARK-D/$name/g" "$HOME/DARK-D/files/.codex.zsh-theme" > "$HOME/.oh-my-zsh/themes/dark.zsh-theme"
    ...
    echo -e "		        ${g}Hey ${y}$name"
    echo -e "${c}              (\_/)"
    echo -e "              (${y}^ω^${c})     ${g}I'm Dark-D699${c}"
    echo -e "             ⊂(___)づ  ⋅˚₊‧ ଳ ‧₊˚ ⋅"
    echo
...
if [ -d "/data/data/com.termux/files/usr/" ]; then
    ...
    echo -e " ${lm}"
    termux
    # darkd check if D1DOS folder exists
    if [ -d "$HOME/DARK" ]; then
        sleep 2
	clear
	banner
